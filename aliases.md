...
| `redup`                                     | Manual, opt-in refresh of fast-moving offensive tools (nuclei templates always + its engine only where that build carries `-update`, which Kali's packaged nuclei does not; katana on the same terms, searchsploit exploit-DB) — attacker box only, never mid-engagement; apt-packaged tools update via `up` |
