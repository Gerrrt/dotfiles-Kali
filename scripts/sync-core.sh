...
#   core_ref      the ref that was pulled (called core_branch before dotfiles-core#453)
...
# Migrate the legacy key BEFORE set_field runs. set_field replaces in place and never
# inserts (see above), so `set_field core_ref` against a lock that still says core_branch
# would match nothing, write nothing, and silently drop the field — leaving a lock this
# very script dies on next run. Renaming the key first means set_field always has a line
# to replace, whichever vintage of lock it was handed. On an already-migrated lock the
# awk matches nothing and this is a no-op.
migrate_branch_to_ref() {
  local t
  t="$(mktemp)"
  awk '{ sub(/^core_branch=/, "core_ref="); print }' "$LOCK" >"$t"
  mv -- "$t" "$LOCK"
}
migrate_branch_to_ref
