...
# The CORE_BRANCH env override keeps its name: it is this script's own knob — set by
# `scripts/sync-core.sh --check` and by the fan-out — not a core.lock field.
...
# Three shapes occur in practice. The field is named for what it holds — the ref that
# was FOLLOWED — which is not always a branch name:
#
...
#
# Both legacy SHAPES are still resolved, deliberately: they exist in this repo's history,
# and a freshness check that died on an old lock would be broken exactly when someone was
# working out how far they had drifted.
#
# Not to be confused with the retired core_branch FIELD (#271). That was an alternative
# NAME for this field and its fallback is gone — a lock without `core_ref=` now dies. What
# survives is tolerance for the three VALUES core_ref can hold, which is a different axis
# and still load-bearing (the resolver below depends on it).
#
